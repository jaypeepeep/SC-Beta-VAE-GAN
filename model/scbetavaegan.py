...
def upload_and_process_files(directory, num_files_to_use=None):
    ...
    if num_files_to_use:
        svc_files = svc_files[
            :num_files_to_use
        ]  # Take the first num_files_to_use files
    ...
    fig, axs = plt.subplots(
        1, num_files, figsize=(6 * num_files, 6), constrained_layout=True
    )
    if num_files == 1:
        axs = [axs]
    ...
    for i, filename in enumerate(svc_files):
        ...
        axs[i].scatter(
            -on_paper["y"], on_paper["x"], c="blue", s=1, alpha=0.7, label="On Paper"
        )
        axs[i].scatter(
            -in_air["y"], in_air["x"], c="red", s=1, alpha=0.7, label="In Air"
        )
    ...
    processed_data = [
        np.column_stack(
            (scaler.transform(df[["x", "y", "timestamp"]]), df["pen_status"].values)
        )
...
def save_original_data(
    data_frames, input_filenames, output_folder="files/original_absolute"
):
...
def process_dataframes(dataframes, num_files_to_use=None):
    ...
    return (
        ...
        scalers,
        avg_data_points,
        [f"DataFrame_{i+1}" for i in range(len(dataframes))],
    )
...
class VAE(tf.keras.Model):
    ...
    def reparameterize(self, mean, logvar):
        ...
        return eps * tf.exp(logvar * 0.5) + (
            mean + self.lambda_shift * tf.exp(logvar * 0.5)
        )
...
def compute_loss(model, x):
    ...
    return (
        reconstruction_loss_xy_timestamp + reconstruction_loss_pen,
        kl_loss,
        model.beta * kl_loss,
    )
...
def generate_augmented_data(
    ...
    avg_data_points,
    processed_data,
    base_latent_variability=1.0,
    latent_variability_range=(0.5, 2.0),
):
    augmented_datasets = []
    ...
    for i in range(num_augmented_files):
        ...
        pressure_azimuth_altitude = original_data[
        ...
        mean, logvar = model.encode(
            tf.convert_to_tensor(selected_data, dtype=tf.float32)
        )
        ...
        augmented_data = np.column_stack(
            (augmented_data, pressure_azimuth_altitude[: augmented_data.shape[0]])
        )
...
def train_vae_step(model, x, optimizer, lstm_discriminator=None):
    with tf.GradientTape() as tape:
        ...
        if lstm_discriminator is not None:
            ...
            fake_predictions = lstm_discriminator(
                tf.expand_dims(x_reconstructed, axis=0)
            )
            ...
            generator_loss = (
                reconstruction_loss + total_kl_loss + 0.1 * discriminator_loss
...
def train_lstm_step(lstm_model, real_data, generated_data, optimizer):
    with tf.GradientTape() as tape:
        ...
        generated_loss = tf.keras.losses.binary_crossentropy(
            tf.zeros_like(generated_predictions), generated_predictions
        )
...
def visualize_augmented_data_from_directory(directory):
    augmented_files = [
        ...
        if f.startswith("augmented_") and f.endswith(".svc")
    ]
    ...
    for i, filename in enumerate(augmented_files):
        ...
        axs[i].scatter(
            on_paper["y"], on_paper["x"], c="b", s=1, alpha=0.7, label="On Paper"
        )
...
def download_augmented_data_with_modified_timestamp(
    all_augmented_data,
    augmented_datasets,
    scalers,
    original_data_frames,
    ...
    for i, (augmented_data, scaler, original_df, original_filename) in enumerate(
        ...
        augmented_data_original_scale = np.column_stack(
            (augmented_xyz_int, pen_status, original_paa_int[: len(augmented_data)])
        )
        ...
        np.savetxt(
            augmented_file_path, augmented_data_original_scale, fmt="%d", delimiter=" "
        )
        np.savetxt(
            nested_file_path, augmented_data_original_scale, fmt="%d", delimiter=" "
        )
        ...
        print(
            f"Shape of augmented data for {original_filename}: {augmented_data_original_scale.shape}"
        )


def nested_augmentation(
    all_augmented_data,
    num_augmentations,
    num_files_to_use,
    pretrained_filename,
    ...
    for iteration in range(num_augmentations):
        ...
        if iteration > 0:
            ...
            (
                ...
                input_filenames,
                original_data_frames,
            ) = upload_and_process_files(directory, num_files_to_use)
...
def calculate_nrmse(original, predicted):
    ...
    if original.shape != predicted.shape:
        raise ValueError(
            "The shapes of the original and predicted datasets must match."
        )
...
def process_files_NRMSE(imputed_folder, augmented_folder, input_filenames):
    ...
    input_filenames_set = {
        os.path.splitext(os.path.basename(filename))[0] for filename in input_filenames
    }
    ...
    for original_file in glob(os.path.join(imputed_folder, "*.svc")):
        ...
        matching_augmented_files = get_matching_augmented_files(
            original_file, augmented_folder
        )
...
def prepare_data_for_lstm(real_data, synthetic_data, add_noise_flag=False):
    ...
    if add_noise_flag:
        real_data_trimmed = add_noise(real_data_trimmed)
        synthetic_data_trimmed = add_noise(synthetic_data_trimmed)
...
def create_lstm_classifier(input_shape):
    # Smaller LSTM model for reduced capacity
    model = Sequential(
        [
            LSTM(64, input_shape=input_shape, return_sequences=True),
            LSTM(32),  # Reduce the number of neurons
...
def create_model(input_shape):
    ...
    model.add(Dropout(0.5))  # Adding dropout to introduce randomness
    model.add(LSTM(100))
